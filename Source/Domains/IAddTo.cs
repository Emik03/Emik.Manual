...
public interface IAddTo
{
    ...
    [Pure]
    static string ToJsonString<T>(in T addTo, IReadOnlyCollection<Region>? regions)
        where T : IAddTo
    {
        JsonNode obj = new JsonObject();
        addTo.CopyTo(ref obj, regions);
    ...
    /// <param name="regions">The regions.</param>
    void CopyTo([NotNullIfNotNull(nameof(value))] ref JsonNode? value, IReadOnlyCollection<Region>? regions);
}
